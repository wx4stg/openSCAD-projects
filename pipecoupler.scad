...
if (!M_TM_GUIDE) {
    ...
    if (PVC_TM_GUIDE) {
        translate([(P_OD/2)+THICK, -(TAPE_MEASUREW/2+THICK), -COUPLER_LENGTH/2]) cube([THICK, TAPE_MEASUREW+THICK*2, COUPLER_LENGTH]);
        translate([(P_OD/2)+THICK, -(TAPE_MEASUREW/2+2*THICK), -COUPLER_LENGTH/2]) cube([TAPE_MEASUREH+THICK, THICK, COUPLER_LENGTH]);
        translate([(P_OD/2)+THICK, (TAPE_MEASUREW/2+THICK), -COUPLER_LENGTH/2]) cube([TAPE_MEASUREH+THICK, THICK, COUPLER_LENGTH]);
...
if (!PVC_TM_GUIDE) {
    ...
    rotate([90, 0, 0]) translate([(P_OD/2)+(M_OD/2)+THICK,0,0]) {
        ...
        if (M_TM_GUIDE) {
            translate([(M_OD/2)+THICK, -(TAPE_MEASUREW/2+THICK), -COUPLER_LENGTH/2]) cube([THICK, TAPE_MEASUREW+THICK*2, COUPLER_LENGTH]);
            translate([(M_OD/2)+THICK, -(TAPE_MEASUREW/2+2*THICK), -COUPLER_LENGTH/2]) cube([TAPE_MEASUREH+THICK, THICK, COUPLER_LENGTH]);
            translate([(M_OD/2)+THICK, (TAPE_MEASUREW/2+THICK), -COUPLER_LENGTH/2]) cube([TAPE_MEASUREH+THICK, THICK, COUPLER_LENGTH]);
